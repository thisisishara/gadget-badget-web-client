...
--
-- Host: 127.0.0.1:3306
-- Generation Time: May 12, 2021 at 09:53 PM
...
--
-- Database: `riwaclientuserservice`
--
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insert_consumer` (IN `username` VARCHAR(40), IN `password` VARCHAR(40), IN `role_id` VARCHAR(10), IN `first_name` VARCHAR(40), IN `last_name` VARCHAR(40), IN `gender` CHAR(1), IN `primary_email` VARCHAR(40), IN `primary_phone` CHAR(12), OUT `result` INT)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insert_employee` (IN `username` VARCHAR(40), IN `password` VARCHAR(40), IN `role_id` VARCHAR(10), IN `first_name` VARCHAR(40), IN `last_name` VARCHAR(40), IN `gender` CHAR(1), IN `primary_email` VARCHAR(40), IN `primary_phone` CHAR(12), IN `gb_employee_id` VARCHAR(10), IN `department` VARCHAR(50), IN `date_hired` DATETIME, OUT `result` INT)  BEGIN
    DECLARE insertVal INTEGER DEFAULT 0;
    DECLARE pref CHAR(2);
    DECLARE date_joined DATETIME;
    DECLARE user_id VARCHAR(10) DEFAULT NULL;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insert_funder` (IN `username` VARCHAR(40), IN `password` VARCHAR(40), IN `role_id` VARCHAR(10), IN `first_name` VARCHAR(40), IN `last_name` VARCHAR(40), IN `gender` CHAR(1), IN `primary_email` VARCHAR(40), IN `primary_phone` CHAR(12), IN `organization` VARCHAR(100), OUT `result` INT)  BEGIN
    DECLARE insertVal INTEGER DEFAULT 0;
    DECLARE pref CHAR(2);
    DECLARE date_joined DATETIME;
    DECLARE user_id VARCHAR(10) DEFAULT NULL;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insert_researcher` (IN `username` VARCHAR(40), IN `password` VARCHAR(40), IN `role_id` VARCHAR(10), IN `first_name` VARCHAR(40), IN `last_name` VARCHAR(40), IN `gender` CHAR(1), IN `primary_email` VARCHAR(40), IN `primary_phone` CHAR(12), IN `institution` VARCHAR(100), IN `field_of_study` VARCHAR(100), IN `years_of_exp` INT(11), OUT `result` INT)  BEGIN
    DECLARE insertVal INTEGER DEFAULT 0;
    DECLARE pref CHAR(2);
    DECLARE date_joined DATETIME;
    DECLARE user_id VARCHAR(10) DEFAULT NULL;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_update_consumer` (IN `uid` VARCHAR(10), IN `username` VARCHAR(40), IN `password` VARCHAR(40), IN `first_name` VARCHAR(40), IN `last_name` VARCHAR(40), IN `gender` CHAR(1), IN `primary_email` VARCHAR(40), IN `primary_phone` CHAR(12), OUT `result` INT)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_update_employee` (IN `uid` VARCHAR(10), IN `username` VARCHAR(40), IN `password` VARCHAR(40), IN `first_name` VARCHAR(40), IN `last_name` VARCHAR(40), IN `gender` CHAR(1), IN `primary_email` VARCHAR(40), IN `primary_phone` CHAR(12), IN `gb_employee_id` VARCHAR(10), IN `department` VARCHAR(50), IN `date_hired` DATETIME, OUT `result` INT)  BEGIN
    DECLARE updateVal INTEGER DEFAULT 0;
    DECLARE `_rollback` BOOL DEFAULT 0;
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION SET `_rollback` = 1;
    START TRANSACTION;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_update_funder` (IN `uid` VARCHAR(10), IN `username` VARCHAR(40), IN `password` VARCHAR(40), IN `first_name` VARCHAR(40), IN `last_name` VARCHAR(40), IN `gender` CHAR(1), IN `primary_email` VARCHAR(40), IN `primary_phone` CHAR(12), IN `organization` VARCHAR(100), OUT `result` INT)  BEGIN
    DECLARE updateVal INTEGER DEFAULT 0;
    DECLARE `_rollback` BOOL DEFAULT 0;
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION SET `_rollback` = 1;
    START TRANSACTION;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_update_researcher` (IN `uid` VARCHAR(10), IN `username` VARCHAR(40), IN `password` VARCHAR(40), IN `first_name` VARCHAR(40), IN `last_name` VARCHAR(40), IN `gender` CHAR(1), IN `primary_email` VARCHAR(40), IN `primary_phone` CHAR(12), IN `institution` VARCHAR(100), IN `field_of_study` VARCHAR(100), IN `years_of_exp` INT, OUT `result` INT)  BEGIN
    DECLARE updateVal INTEGER DEFAULT 0;
    DECLARE `_rollback` BOOL DEFAULT 0;
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION SET `_rollback` = 1;
    START TRANSACTION;
...
('AD21000001', 'GB21000002', 'IT Department', '2015-02-21 00:00:00'),
('AD21000028', 'GB21000004', 'IT', '2020-02-21 00:00:10'),
('AD21000093', 'GB66554422', 'Finance Department', '2021-04-25 00:00:00'),
('EM21000018', 'GB21000005', 'Marketing', '2020-02-04 08:00:00'),
('FM21000027', 'GB21000006', 'Finance', '2010-02-21 10:00:00'),
('FM21000094', 'GB66666667', 'IT Department', '2019-06-28 00:00:00');
...
('RS21000041', 'SLIIT', 'Computational Linguistics', 3),
('RS21000042', 'SLIIT Academmy', 'Distributed & Parallel Computing', 5),
('RS21000043', 'UoC', 'Data Communication & Networking', 5),
('RS21000096', 'res2', 'Business Intelligence and Analytics', 4);
...
CREATE TABLE IF NOT EXISTS `user` (
  ...
  `primary_phone` char(12) DEFAULT NULL,
...
('AD21000028', 'dave114', 'd@v311A', 'ADMIN', 'David', 'Jackson', 'M', '2021-04-18 18:44:52', 'dave.j@gadgetbadget.com', '0710000003', 'Yes'),
('AD21000093', 'empint1', 'empint1', 'ADMIN', 'empint1f', 'empint1l', 'O', '2021-05-10 15:09:23', 'empint1@empint1.co', '+94555222666', 'Yes'),
('CN21000016', 'katiejohnes', 'ktj@123', 'CNSMR', 'Katie', 'Johnes', 'M', '2021-04-18 15:39:29', 'ktj@yahoo.com', '0710000004', 'No'),
('CN21000088', 'conext1', 'conext1', 'CNSMR', 'conextf1f', 'conextl1l', 'F', '2021-05-10 14:54:03', 'conext1@conext.co', '0111222334', 'Yes'),
...
('FM21000094', 'empint23', 'empint2', 'FNMGR', 'empint2ff', 'empint2ll', 'M', '2021-05-10 15:10:51', 'empint2@my.com', '0222555889', 'No'),
...
('FN21000033', 'arther_96', 'c@llme4979', 'FUNDR', 'Arthur', 'Rodriguz', 'M', '2021-04-18 19:05:26', 'cotact@artheristheman.com', '0710000010', 'Yes'),
('FN21000090', 'funint1un', 'funint1', 'FUNDR', 'funint1fir', 'funint1lst', 'F', '2021-05-10 15:04:26', 'funint1@funint1.com', '0712225557', 'Yes'),
('RS21000041', 'janedoe', 'jane@123', 'RSCHR', 'Jane', 'Doe', 'F', '2021-04-18 19:27:28', 'janedoe@gmail.com', '0710000002', 'Yes'),
('RS21000042', 'peteross9', 'unH@ck4b*3', 'RSCHR', 'Peter', 'Ross', 'M', '2021-04-18 19:27:54', 'pete.ross@gmail.com', '0710000012', 'Yes'),
('RS21000043', 'nathon_black', 'nathonBLACK@17', 'RSCHR', 'Nathan', 'Blackmirror', 'M', '2021-04-18 19:27:55', 'itsblack@gmail.com', '0710000013', 'Yes'),
('RS21000096', 'res2', 'res2', 'RSCHR', 'res2', 'res2', 'F', '2021-05-13 03:13:37', 'res2@res2.res', '+94999666555', 'No');
