...
public class Consumer extends User{
	...
	public JsonObject readConsumers() {
		...
		try
		{
			...
			if(!rs.isBeforeFirst()) {
				return new JsonResponseBuilder().getJsonFailedResponse("Request Processed. No Consumers found.");
			}
	...
	//Read Consumers By Id
	public JsonObject readConsumerById(String consumer_id) {
		JsonObject result = null;
		try
		{			
			...
			if(!rs.isBeforeFirst()) {
				return new JsonResponseBuilder().getJsonFailedResponse("Request Processed. Consumer not found.");
			}
		...
		{
			System.err.println(ex.getMessage());
			return new JsonResponseBuilder().getJsonExceptionResponse("Error occurred while reading consumer. Exception Details:" + ex.getMessage());
		}
	}
